...
import 'package:tictactoe_flutter/utils/gamedialog.dart';
...
class GameMethods {
  void checkWinner(BuildContext context, Socket socketClent) {
    ...
    if (roomDataProvider.displayElements[2] ==
      ...
      winner = '';
      showGameDialog(context, 'Result: Draw');
    }

    if (winner != '') {
      if (roomDataProvider.player1.playerType == winner) {
        print(roomDataProvider.player1.username);
        showGameDialog(context, '${roomDataProvider.player1.username} won!');
        socketClent.emit('winner', {
          'winnerSocketId': roomDataProvider.player1.socketID,
          'roomId': roomDataProvider.roomData['_id'],
        });
      } else {
        print(roomDataProvider.player2.username);
